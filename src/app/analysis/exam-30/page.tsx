  ...
  const studyPriorities = [
    {
      ...
      strategy:
        '第30回では弁証論治を中心とした東洋医学臨床論がPM専門科目の最多テーマ（Q127〜Q160の34問）。緊張型頭痛・神経根症・顎関節症・月経痛・スポーツ障害・腰痛・浮腫など幅広い症例統合問題が出題。「四診→弁証→取穴」の思考過程を実践的に演習することが最優先。',
    },
    {
      rank: 2,
      ...
      badge: 'S' as Importance,
      strategy:
        'はり理論（Q161〜170）・きゅう理論（Q171〜180）は各10問・合計20問。はり理論では十二刺の偶刺・古代九鍼（鋒鍼＝三稜鍼）・打鍼法（円鍼形・御園意斎）・折鍼リスク（細径鍼・長時間置鍼）・低周波鍼通電療法の禁忌・ポリモーダル受容器・脊髄分節性鎮痛。きゅう理論では艾の製造工程（けんどん）・無痕灸の種類・熱傷深度・アラキドン酸代謝（COX/LOX経路）・局所炎症反応（CGRP・フレア）が出題。',
    },
    {
      rank: 3,
      ...
      badge: 'S' as Importance,
      strategy:
        '経絡経穴概論は20問出題（Q107〜Q126）。奇経八脈（陽跷脈の陽気調節・申脈起始）・八会穴と募穴の重複（中脘・膻中・章門）・骨度法（各部の寸数）・経穴の正確な部位（陽渓＝鼻煙窩・第4〜5中手骨間の液門・中渚）・六つ灸と膏肓の同高さ・五兪穴の難経適用（虚補母・実瀉子）。部位問題は解剖学的ランドマークと照合して暗記する。',
    },
    {
      rank: 4,
      ...
      <section>
        ...
        <div className="bg-green-50 border border-green-100 rounded-xl p-5 space-y-3 text-sm text-gray-700 leading-relaxed">
          <p>
            ...
            が第30回の最大テーマ。Q127〜Q160の34問（東洋医学臨床論＋午後総合問題）が症例型弁証論治問題となっており、月経痛・上腹部痛・スポーツ障害・腰痛・浮腫・顔面神経麻痺・神経根症状など実践的な臨床シナリオから証を特定して治療穴を選択するパターンが特徴的。
          </p>
          <p>
            <strong className="text-green-700">PM専門科目（はり・きゅう + 経絡経穴 + 東洋医学）</strong>
            の合計は{pmSpecCount}問（{pct(pmSpecCount, total)}%）で全体の約半数を占める。
            鍼灸専門知識の深さが合否を分ける構成であり、東洋医学臨床論（Q127〜Q160＝34問）が専門科目の中心。
          </p>
          <p>
