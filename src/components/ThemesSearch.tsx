...
function themeMatches(t: Theme, q: string): boolean {
  if (!q) return true
...
export default function ThemesSearch({ themes, subjects, examStats }: Props) {
  ...
  const filtered = useMemo(() => {
    return themes
      ...
      .sort((a, b) => {
        if (sortBy === 'count') {
          return statsOf(b.id).examRounds.length - statsOf(a.id).examRounds.length ||
            importanceOrder[a.importance] - importanceOrder[b.importance]
        }
        if (sortBy === 'importance') {
          return importanceOrder[a.importance] - importanceOrder[b.importance] ||
            statsOf(b.id).examRounds.length - statsOf(a.id).examRounds.length
        }
        return statsOf(b.id).latestRound - statsOf(a.id).latestRound ||
          statsOf(b.id).examRounds.length - statsOf(a.id).examRounds.length
      })
  }, [themes, query, selectedSubject, selectedImportance, sortBy, statsOf])
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-shrink-0 text-right">
                    <p className="text-lg font-bold text-green-700">{stats.examRounds.length}回</p>
                    <p className="text-xs text-gray-400">{stats.latestRound ? `第${stats.latestRound}回` : '出題実績なし'}</p>
                  </div>
                </div>

                <div className="mt-3 flex gap-1.5 flex-wrap">
                  {allRounds.map(round => {
                    const hit = stats.examRounds.includes(round)
                    return (
                      <span
                        key={round}
